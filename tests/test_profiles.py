...
async def test_change_username(user_app: AsyncClient, user: User) -> None:
    response = await user_app.put("/profiles/username", json={"username": "another_user"})
    assert response.status_code == 200, response.json()
    assert response.json()["username"] == "another_user"


async def test_change_username_exists(user_app: AsyncClient, user: User) -> None:
    response = await user_app.put("/profiles/username", json={"username": "user"})
    assert response.status_code == 422
    assert response.json()["detail"] == f"User user already exists."


async def test_change_email(user_app: AsyncClient, user: User, new_email: EmailStr) -> None:
...
async def test_change_password(
        user_app: AsyncClient, user: User, plain_password: str, new_plain_password: str
) -> None:
    response = await user_app.put(
        "/profiles/password",
        json={"old_password": plain_password, "new_password": new_plain_password}
    )
    assert response.status_code == 200, response.json()
    verification = VerificationOut(**response.json())
    assert verify_password(new_plain_password, verification.action.data.password)
