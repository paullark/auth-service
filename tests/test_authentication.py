...
async def test_signup(app: AsyncClient, signup_data: SignupData, db: Database) -> None:
    ...
    await db.find(
        Verification, {"_id": ObjectId(response.json().get("_id"))}, exception=True
    )
...
async def test_login_incorrect_password(app: AsyncClient, login_data: LoginData) -> None:
...
async def test_refresh(user_app: AsyncClient, user_token_pair: TokenPair) -> None:
    response = await user_app.post(
